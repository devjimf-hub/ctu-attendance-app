...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess }) => {
  ...
  const [department, setDepartment] = useState('College Faculty / IT Department');

  const handleCustomLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;
    const user = authService.login(name, email, department);
    onLoginSuccess(user);
  };
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', background: 'var(--bg-main)' }}>
      <div style={{ width: '100%', maxWidth: '420px', background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-md)', padding: '2rem 1.5rem', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '1.75rem' }}>
          Teacher & Professor Attendance Portal
        </p>

        {/* 1-Tap Quick Demo Login (Great for instant testing / offline) */}
        <button
        ...
        <div style={{ display: 'flex', alignItems: 'center', margin: '1rem 0', color: 'var(--text-muted)', fontSize: '0.75rem' }}>
          ...
          <span style={{ padding: '0 0.6rem' }}>OR SIGN IN WITH CUSTOM NAME</span>
          <hr style={{ flex: 1, borderColor: 'var(--border-color)' }} />
        </div>
        ...
        <div style={{ marginTop: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
          ...
          <span>Offline Ready • No connection required</span>
        </div>
      </div>
    </div>
  );
